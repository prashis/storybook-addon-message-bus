...
import { ResetWrapper, Form, ObjectControl } from "@storybook/components";
import { Event, OnEmitEvent } from "../typings";

type PanelContentProps = {
  onEmit?: (x: OnEmitEvent) => void;
  events?: Event[];
};

type PanelContentRowProps = {
  name: Event["name"];
...
const Row: React.FC<PanelContentRowProps> = ({ name, payload, onEmit }) => {
  ...
  const onChange = useCallback((newPayload: Event["payload"]) => {
    setValue(newPayload);
    return newPayload;
  }, []);
...
export const PanelContent: React.FC<PanelContentProps> = ({
  ...
  return (
    <ResetWrapper>
      <TableWrapper aria-hidden={false}>
        <thead>
          <tr>
            <th>
              <span>Event</span>
        ...
        <tbody>
          {events.map(({ name, payload }, index) => (
            <Row key={`control-row-${index}`} {...{ name, payload, onEmit }} />
          ))}
        </tbody>
      </TableWrapper>
    </ResetWrapper>
